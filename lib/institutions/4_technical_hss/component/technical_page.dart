import 'package:flutter/material.dart';
import 'package:flutter_application_1/institutions/components/custom_appbar/institutiondetail_appbar.dart';
import 'package:flutter_application_1/institutions/institution_class.dart';
import 'package:flutter_application_1/institutions/components/info_card.dart';

class TechnicalPage extends StatefulWidget {
  final TechnicalHSS school;

  const TechnicalPage({Key? key, required this.school}) : super(key: key);

  @override
  TechnicalPageState createState() => TechnicalPageState();
}

class TechnicalPageState extends State<TechnicalPage> {
  final double _appBarHeight = 240.0;
  final double _collapsedAppBarHeight = 80.0;
  bool _isScrolled = false;
  final ScrollController _scrollController = ScrollController();

  @override
  void dispose() {
    _scrollController.dispose();
    super.dispose();
  }

  @override
  void initState() {
    super.initState();
    _scrollController.addListener(() {
      if (_scrollController.offset >= _appBarHeight - _collapsedAppBarHeight) {
        setState(() {
          _isScrolled = true;
        });
      } else {
        setState(() {
          _isScrolled = false;
        });
      }
    });
  }

  @override
  Widget build(BuildContext context) {
    final secondList = widget.school.secondaryCourses?.map((e) => ListTile(
          leading: const SizedBox(
            width: 20, // Set the width to a smaller value
            child: Icon(
              Icons.fiber_manual_record_sharp,
              color: Colors.black,
              size: 13,
            ),
          ),
          title: Text(
            e.name,
            style: const TextStyle(fontSize: 14, color: Colors.black),
          ),
          trailing: Text(
            e.seats ?? "",
            style: const TextStyle(fontSize: 14, color: Colors.black),
          ),
        ));

    final hsList = widget.school.hsCourses.map((e) => ListTile(
          leading: const SizedBox(
            width: 20, // Set the width to a smaller value
            child: Icon(
              Icons.fiber_manual_record_sharp,
              color: Colors.black,
              size: 13,
            ),
          ),
          title: Text(
            "${e.name}: ${e.info ?? ""}",
            style: const TextStyle(fontSize: 14, color: Colors.black),
          ),
          trailing: Text(
            e.seats ?? "",
            style: const TextStyle(fontSize: 14, color: Colors.black),
          ),
        ));

    final ihrdCourseList = widget.school.ihrdCourses?.map((e) => ListTile(
          leading: const CircleAvatar(child: Text('*')),
          title: Text(
            e,
            style: const TextStyle(fontSize: 14, color: Colors.black),
          ),
        ));

    BoxDecoration cardBg = BoxDecoration(
      color: Colors.white,
      boxShadow: const [
        BoxShadow(
          color: Colors.black12,
          blurRadius: 10,
          offset: Offset(0, 5),
        ),
      ],
      borderRadius: BorderRadius.all(Radius.circular(20)),
      border: Border.all(
        color: Color(0x5486C4F3),
        width: 2,
      ),
    );
    return Scaffold(
      body: CustomScrollView(
        controller: _scrollController,
        slivers: [
          SliverAppBar(
            //Statusbar height
            toolbarHeight: MediaQuery.of(context).padding.top + 40,
            backgroundColor: _isScrolled
                ? Color.fromRGBO(11, 162, 213, 0.847)
                : Colors.transparent,
            expandedHeight: _appBarHeight,
            pinned: true,
            flexibleSpace: LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                return ClipRRect(
                  borderRadius: BorderRadius.only(
                    bottomLeft: Radius.circular(40.0),
                    bottomRight: Radius.circular(40.0),
                  ),
                  child: FlexibleSpaceBar(
                    title: !_isScrolled
                        ? null
                        : Text(
                            widget.school.name,
                            style: TextStyle(fontSize: 15),
                          ),
                    background: CustomAppBar(
                      collegeName: widget.school.name,
                      collegeAddress: widget.school.address,
                    ),
                    collapseMode: CollapseMode.parallax,
                    stretchModes: const [
                      StretchMode.zoomBackground,
                      StretchMode.blurBackground,
                      StretchMode.fadeTitle,
                    ],
                  ),
                );
              },
            ),
          ),
          SliverList(
            delegate: SliverChildListDelegate(
              [
                const SizedBox(height: 30),
                InfoCard(
                    content: Text(
                      'Phone: ${widget.school.phone}\nEmail: ${widget.school.email}\nWebsite: ${widget.school.website}',
                      style: const TextStyle(
                          fontSize: 13, color: Colors.black, height: 1.8),
                    ),
                    title: "Contact Details",
                    icon: const Icon(Icons.phone)),
                const SizedBox(height: 20),
                InfoCard(
                    content: Text(
                      widget.school.about,
                      style: const TextStyle(
                          fontSize: 13, color: Colors.black, height: 1.54),
                    ),
                    title: "About",
                    icon: const Icon(Icons.info)),
                const SizedBox(height: 20),
                InfoCard(
                    content: hsList.isNotEmpty
                        ? Column(
                            children: hsList.toList(),
                          )
                        : const Text(
                            'No Courses Available',
                            style: TextStyle(fontSize: 13, color: Colors.black),
                          ),
                    title: "Higher Secondary Level",
                    icon: const Icon(Icons.book)),
                const SizedBox(height: 20),
                InfoCard(
                    content: secondList != null
                        ? Column(
                            children: secondList.toList(),
                          )
                        : const Text(
                            'No Courses Available',
                            style: TextStyle(fontSize: 13, color: Colors.black),
                          ),
                    title: "Secondary Level",
                    icon: const Icon(Icons.book)),
                const SizedBox(height: 20),
                InfoCard(
                    content: ihrdCourseList != null
                        ? Column(
                            children: ihrdCourseList.toList(),
                          )
                        : const Text(
                            'No Courses Available',
                            style: TextStyle(fontSize: 13, color: Colors.black),
                          ),
                    title: "IHRD Courses",
                    icon: const Icon(Icons.book)),
                const SizedBox(height: 20),
                InfoCard(
                    content: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          'Nearest Railway Station: ${widget.school.nearestRailwayStation}',
                          style: const TextStyle(
                              fontSize: 14, color: Colors.black),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Nearest Bus Station: ${widget.school.nearestBusStand}',
                          style: const TextStyle(
                              fontSize: 14, color: Colors.black),
                        ),
                      ],
                    ),
                    title: "Nearest Stations",
                    icon: const Icon(Icons.map)),
                const SizedBox(height: 20),
              ],
            ),
          ),
        ],
      ),
    );
  }
}
